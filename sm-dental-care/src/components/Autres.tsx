...
//     return date.toLocaleDateString("en-GB", options); // ex: "26/02/2025 (Wednesday)"
//   };
...
//                  {/* Graphique du nombre de paiements par jour */}
//         <div className="bg-white p-6 rounded-lg shadow-md">
//            <h2 className="text-xl font-semibold mb-4">Nombre de Paiements par Jour</h2>
//           <ResponsiveContainer width="100%" height={300}>
//              <BarChart data={paymentData}>              <CartesianGrid strokeDasharray="3 3" />
...
const Autres: React.FC = () => {
  ...
  const getTreatmentStatistics = () => {
    ...
    return { totalTreatments,treatmentsToday, treatmentsThisWeek, treatmentsThisMonth, treatmentsThisYear };
  };
  ...
  const processData = (data: any[], key: string) =>
    Object.values(
      data.reduce((acc, item) => {
        const date = formatDateWithDay(item.date || "Inconnue");
        acc[date] = acc[date] || { date, count: 0, totalPaid: 0 };
        acc[date].count += 1;
        if (key === "payments") acc[date].totalPaid += item.paid || 0;
        return acc;
      }, {} as Record<string, { date: string; count: number; totalPaid: number }>)
    );

  const patientData = processData(patients, "patients");
  const treatmentData = processData(treatments, "treatments");
  const paymentData = processData(payments, "payments");

  const paymentPieData = [
    { name: "Total Paiements", value: payments.reduce((sum, p) => sum + p.paid, 0) },
    { name: "Reste à Payer", value: Math.max(0, treatments.reduce((sum, t) => sum + t.price, 0) - payments.reduce((sum, p) => sum + p.paid, 0)) },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-[#0C1B33]">
      <Sidebar />
              ...
              <div className="flex-1 p-6 space-y-6">
        <h1 className="text-3xl font-bold text-slate-300">Statistiques du Cabinet Dentaire</h1>
 {/* Payment & Treatment Statistics Section */}
<div className="bg-[#B2AA8E] p-6 shadow-lg rounded-lg my-6">
  <h2 className="text-xl font-semibold mb-4">Statistiques Financières et Médicales</h2>
   <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
    ...
    {/* Today's Payments */}
     <div className="p-4 bg-blue-100 rounded-lg">
     <span className="text-sm text-gray-500">Paiements Aujourd'hui</span>
      <h4 className="text-xl font-bold text-blue-600">{getPaymentStatistics().paymentsToday.toFixed(2)} DZD</h4>    </div>
 ...
 {/* Treatments Total */}
 <div className="p-4 bg-blue-100 rounded-lg">
       <span className="text-sm text-gray-500">Traitements Total</span>
       <h4 className="text-xl font-bold text-cyan-600">{getTreatmentStatistics().totalTreatments}</h4>
     </div>
     ...
     <div className="p-4 bg-red-100 rounded-lg">
       ...
       <h4 className="text-xl font-bold text-red-600">{getTreatmentStatistics().treatmentsThisYear}</h4>
        ...
        {/* Patients Chart */}
        <div className="bg-[#fff5d5] p-6 rounded-lg shadow-md my-3">
          <h2 className="text-xl font-semibold mb-4">Évolution du Nombre de Patients</h2>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={patientData}>
              <CartesianGrid strokeDasharray="3 3" />
        ...
        <div className="bg-[#fff5d5] p-6 rounded-lg shadow-md my-3">
          <h2 className="text-xl font-semibold mb-4">Nombre de Paiements</h2>
          <ResponsiveContainer width="100%" height={300}>
