...
const PatientForm: React.FC<PatientFormProps> = ({ patient, onCancel, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
      <div className="bg-[#B2AA8E] p-6 rounded-lg shadow-lg w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-[#0C1B33]" id="dialog-title">
        ...
        <form onSubmit={handleSubmit} className="space-y-2" aria-labelledby="dialog-title">
          <div>
            <label htmlFor="patient-name" className="block text-sm font-medium text-gray-700">
              Name
            </label>
            <input
              ...
              value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              className="mt-1 block w-full px-4 py-1 rounded-md border-[2px] border-[#0C1B33] shadow-sm focus:border-[#1d3c6e] focus:ring-[#1d3c6e]"
              required
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setFormData({ 
                ...formData, 
                age: e.target.value ? parseInt(e.target.value, 10) : 0 
              })}
              className="mt-1 block w-full px-4 py-1 rounded-md border-[2px] border-[#0C1B33] shadow-sm focus:border-[#1d3c6e] focus:ring-[#1d3c6e]"
          ...
          <div>
            ...
            <textarea
              ...
              className="mt-1 block w-full px-4 py-1 rounded-md border-[2px] border-[#0C1B33] shadow-sm focus:border-[#1d3c6e] focus:ring-[#1d3c6e]"
              rows={3}
              placeholder="Describe the patient's case"
            />
          </div>
          ...
          <div className="flex justify-end space-x-3">
            ...
            <button
              type="submit"
              className="px-4 py-2 text-sm font-medium text-white bg-[#0C1B33] rounded-md hover:bg-[#1d3c6e]"
            >
              {patient ? 'Update' : 'Add'}
            </button>
          </div>
